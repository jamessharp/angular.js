...
describe("resource", function() {
  ...
  describe("with rootKey", function() {
    beforeEach(inject(function($injector) {
      $httpBackend = $injector.get('$httpBackend');
      $resource = $injector.get('$resource');
      CreditCard = $resource({
        url: '/CreditCard/:id:verb',
        rootKey: 'card',
        paramDefaults: {id:'@id.key'},
        actions: {
          charge: {
            method:'post',
            params:{verb:'!charge'}
    ...
    it("should create resource with the rootKey in post data", function() {
      $httpBackend.expect('POST', '/CreditCard', {card: {name:"misko"}}).respond({card: {id: 123, name: 'misko'}});
    ...
    it("should read resource from the property with rootKey", function() {
      $httpBackend.expect('GET', '/CreditCard/123').respond({card: {id: 123, number: '9876'}});
      var cc = CreditCard.get({id: 123}, callback);
    ...
    it("should query resource reading the plural form of rootKey", function() {
      $httpBackend.expect('GET', '/CreditCard?key=value').respond({cards: [{id: 1}, {id: 2}]});
    ...
    it("should query resource reading the plural form of rootKey when explicitly specified", function() {
      $httpBackend.expect('GET', '/CreditCard?key=value').respond({wibbles: [{id: 1}, {id: 2}]});
      var FunkyCreditCard = $resource({
        url: '/CreditCard/:id:verb',
        rootKey: 'card',
        rootKeyPlural: 'wibbles'
      });
      var ccs = FunkyCreditCard.query({key: 'value'}, callback);
      expect(ccs).toEqual([]);
      expect(callback).not.toHaveBeenCalled();
  ...
  describe("with idKey", function() {
    ...
    it('should create resource and any further requests for that id will return the same object', function() {
      $httpBackend.expect('POST', '/CreditCard', {name:"misko"}).respond({id: {key: 123}, name: 'misko'});
      ...
      $httpBackend.expect('GET', '/CreditCard/123').respond({id: {key: 123}, name: 'misko2'});
      var cc2 = CreditCard.get({id: 123});
      expect(cc2).toBe(cc);
      $httpBackend.flush();
      expect(cc).toEqualData({id: {key:123}, name: 'misko2'});
    });

    it('should create resource directly and any further requests for that id will return the same object', function() {
      $httpBackend.expect('POST', '/CreditCard', {name:"misko"}).respond({id: {key: 123}, name: 'misko'});

      var cc = new CreditCard({name: 'misko'});
      cc.$save(callback);
      expect(cc).toEqualData({name: 'misko'});
      expect(callback).not.toHaveBeenCalled();
      $httpBackend.flush();
    ...
    it('should return the same object from a query or a get where the ids match', function() {
      $httpBackend.expect('POST', '/CreditCard', {name:"misko"}).respond({id: {key: 1}});

      var cc = CreditCard.save({name: 'misko'}, callback);
      $httpBackend.flush();
      expect(cc).toEqualData({id: {key:1}});
      expect(callback).toHaveBeenCalledOnce();
      expect(callback.mostRecentCall.args[0]).toEqual(cc);
      expect(callback.mostRecentCall.args[1]()).toEqual({});

      $httpBackend.expect('GET', '/CreditCard?key=value').respond([{id: {key: 1}}, {id: {key: 2}}]);

      var ccs = CreditCard.query({key: 'value'}, callback);
      expect(ccs).toEqual([]);

      $httpBackend.flush();
      expect(ccs).toEqualData([{id: {key: 1}}, {id: {key: 2}}]);
      expect(callback.mostRecentCall.args[0]).toEqual(ccs);
      expect(callback.mostRecentCall.args[1]()).toEqual({});

      expect(ccs[0]).toBe(cc);
    });
  });
});
