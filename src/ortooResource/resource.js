...
/**
 * @ngdoc overview
 * @name ngOrtooResource
 * @description
 */

/**
 * @ngdoc object
 * @name ngOrtooResource.ortooResource
 * @requires $http
 *
 * @description
 ...
 *   number, like this: `ortooResource('http://example.com\\:8080/api')`.
 *
 ...
 *   <pre>
        var User = ortooResource('/user/:userId', {userId:'@id'});
 ...
 *
 *   It is important to realize that invoking a ortooResource object method immediately returns an
 ...
 * <pre>
     ...
     var CreditCard = ortooResource('/user/:userId/card/:cardId',
 ...
 * # Buzz client
   ...
   Let's look at what a buzz client created with the `ortooResource` service looks like:
    <doc:example>
      <doc:source jsfiddle="false">
       <script>
         function BuzzController(ortooResource) {
           this.userId = 'googlebuzz';
           this.Activity = ortooResource(
         ...
         BuzzController.$inject = ['ortooResource'];
       </script>
...
angular.module('ngOrtooResource', ['ng']).
  factory('ortooResource', ['$http', '$parse', '$log', function($http, $parse, $log) {
    ...
    function ResourceFactory(url, rootKey, idKey, paramDefaults, actions) {
      ...
      if (rootKey) {
        if ((typeof rootKey === 'object') && !!rootKey.single && !!rootKey.plural) {
          rootKey = rootKey.single;
          rootKeyPlural = rootKey.plural;
        } else if (typeof rootKey === 'string') {
          rootKeyPlural = rootKey + 's';
        } else {
          throw 'Expected rootKey to be a string or object of form { single: <key>, plural: <keys> }';
        }
      }
